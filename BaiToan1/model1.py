...
columns_to_rank = ['Inlab1']
for column in columns_to_rank:
    labDF[column] = labDF[column].apply(ranking)
labDF = labDF[labDF['Prelab1-attempts']>5]
# Pre1 => lab1
X1 = labDF[['Prelab1', 'Prelab1-attempts', 'Prelab1-questions','Prelab1-growths']]
Y1 = labDF['Inlab1']
...
best_rf_model.fit(X_train1_scaled, y_train1)

# Dự đoán và tính toán độ chính xác
y_pred = best_rf_model.predict(X_test1_scaled)
