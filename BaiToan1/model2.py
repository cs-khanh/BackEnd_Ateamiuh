...
columns_to_rank = ['Inlab1','Inlab2']
for column in columns_to_rank:
    labDF[column] = labDF[column].apply(ranking)

X2 = labDF[['Prelab1', 'Prelab1-attempts', 'Prelab1-questions',
            'Prelab1-growths', 'Inlab1','Inlab1-attempts', 
            'Inlab1-questions', 'Inlab1-growths', 'Prelab2', 
            'Prelab2-attempts','Prelab2-questions','Prelab2-growths']]
Y2 = labDF['Inlab2']
X_train2, X_test2, y_train2, y_test2 = train_test_split(X2, Y2,stratify=Y2, test_size=0.1, random_state=42)
print(X_train2)
print(X_test2)
scaler = MinMaxScaler()
...
print(X_test2_scaled)
best_rf_model.fit(X_train2_scaled, y_train2)
y_pred = best_rf_model.predict(X_test2_scaled)
accuracy_Random2 = accuracy_score(y_test2, y_pred)
print("Accuracy for Random Forest with MinMax Scaling:", accuracy_Random2)

# Lưu mô hình và scaler vào file
model_filename = "ModelsBaiToan1/Model2/best_rf_model2_with_scaler.pkl"
joblib.dump(best_rf_model, model_filename)
scaler_filename = "ModelsBaiToan1/Model2/scaler2.pkl"
